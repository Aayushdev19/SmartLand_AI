...
def build_features(district, locality, area_category, land_type, jantri_price, year):
...
def investment_score(market_price, jantri_price, district, land_type):
...
def risk_level(market_price, jantri_price):
...
def price_forecast(current_price, growth_rate=0.08):
    return {
...
class PredictRequest(BaseModel):
    ...
    jantri_price:  float = Field(..., example=8500)
    year:          int   = Field(2024, example=2024)
    area_sqm:      Optional[float] = Field(None, example=150)
...
def analytics_trends():
    return {
        "yearly_trend": analytics_data["jantri_vs_market"],
